...
program skyrmionlattice
    ...
    !##########################################
    integer, parameter :: L = 20, N = L*L, M = 2*N, num_T_para = 20, num_MC = 4*10**4, nonzeros = 8*M, cutoff_dos = 100
    real*8, parameter :: t = 1.0, PI = 3.141592654
    integer :: i, j, k, ii, r, d, di, dd, ij, aver_num, cutoff
    integer :: p, p_i_pl, p_i_mi, p_j_pl, p_j_mi, p_i_pl_j_mi, p_i_mi_j_pl
    real*8 :: ran, J_k, beta, miu, Tc, x, Weight_fermi, Weight_fermi_new, Emax, Emin, a, b, t1, t2
	...
	real*8, dimension(cutoff_dos) :: moments
...
do ii = 1, num_T_para
	...
	call pem_logweight( cutoff, M, nonzeros, val1, val2, colind, rowptr, beta, miu, a, b, Weight_fermi )
    ...
    do i = 1, num_MC
        ...
        H(r, r) = ( -J_k*cos(theta(dd,r))-b )/a
        H(r+N, r+N) = ( J_k*cos(theta(dd,r))-b )/a
        H(r+N, r) = -J_k*sin(theta(dd,r))*exp( cmplx(0.0, fai(dd,r)) )/a
        H(r, r+N) = -J_k*sin(theta(dd,r))*exp( cmplx(0.0, fai(dd,r)*(-1)) )/a
        call pem_sparse( H, val1, val2, colind, rowptr , M, nonzeros)
        call pem_logweight( cutoff, M, nonzeros, val1, val2, colind, rowptr, beta, miu, a, b, Weight_fermi_new )
        ...
        if ( x > exp( Weight_fermi_new-Weight_fermi-beta*(Hp-Hc(r)+deltaV) ) ) then
            ...
            H(r, r) = ( -J_k*cos(theta(d,r))-b )/a
            H(r+N, r+N) = ( J_k*cos(theta(d,r))-b )/a
            H(r+N, r) = -J_k*sin(theta(d,r))*exp( cmplx(0.0, fai(d,r)) )/a
            H(r, r+N) = -J_k*sin(theta(d,r))*exp( cmplx(0.0, fai(d,r)*(-1)) )/a
        else
        ...
        if ( i > num_MC-aver_num*N ) then
            if ( mod(i+aver_num*N-num_MC, N) == 0 ) then
        		...
        		call pem_sparse( H, val1, val2, colind, rowptr , M, nonzeros)
				call pem_moments( cutoff_dos, M, nonzeros, val1, val2, colind, rowptr, moments)
				write(16,*) moments
            end if
        end if
        write(13,*) V
